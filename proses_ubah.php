...
if(isset($_POST['ubah_gambar'])){ // Jika user menceklis checkbox yang ada di form ubah, lakukan :
	...
	if(move_uploaded_file($tmp, $path)){ // Cek apakah gambar berhasil diupload atau tidak
		...
		$query = "UPDATE wisata SET nama_wisata='".$nama_wisata."',  deskripsi='".$deskripsi."', gambar='".$gambarbaru."' WHERE id='".$id."'";
		$sql = mysqli_query($connect, $query); // Eksekusi/ Jalankan query dari variabel $query
	...
	// Proses ubah data ke Database
	$query = "UPDATE wisata SET nama_wisata='".$nama_wisata."',  deskripsi='".$deskripsi."' WHERE id='".$id."'";
	$sql = mysqli_query($connect, $query); // Eksekusi/ Jalankan query dari variabel $query
